...
function boxedJavaType(type) {
    ...
    if (isRecordType(type))
        return `Map<String, ${boxedJavaType(recordElementType(type))}>`;
...
function generateArrayRead(type, r, targetVar, indent) {
    const elem = arrayElementType(type);
...
function generateRecordRead(type, r, targetVar, indent) {
    const elem = recordElementType(type);
    const javaElem = boxedJavaType(elem);
    const lines = [];
    lines.push(`${indent}{`);
...
export async function $onEmit(context) {
    ...
    for (const svc of services) {
        ...
        lines.push(``);
        lines.push(`public class ${toPascalCase(toSnakeCase(svc.serviceName))}Types {`);
        ...
        lines.push(`}`);
        lines.push(``);
        const fileName = `${toPascalCase(toSnakeCase(svc.serviceName))}Types.java`;

...
export function collectServices(program) {
    ...
    function collectDirectModels(ns) {
        const models = [];
    ...
    function collectDirectEnums(ns) {
        const enums = [];
        for (const [, type] of ns.enums) {
            if (type.kind === "Enum" && type.name && isCodecType(program, type)) {
                const e = type;
                const members = [];
                let autoValue = 0;
    ...
    function collectDirectUnions(ns) {
        const unions = [];
        for (const [, type] of ns.unions) {
            if (type.kind === "Union" && type.name && isCodecType(program, type)) {
                const u = type;
                const variants = [];
    ...
    function fakeIface(name, ns) {
        return { name, namespace: ns };
    }
    function addService(ns, serviceName) {
        if (pathsSeen.has(serviceName))
            return;
    ...
    function walkNamespace(ns, path) {
...
export function isModelType(type) {
    return type.kind === "Model" && !!type.name && !isArrayType(type) && !isRecordType(type);
}
export function isUnionType(type) {
    return type.kind === "Union" && !!type.name;
}
export function isScalarVariant(type) {
    if (type.kind === "Scalar")
        return true;
    if (type.kind === "Enum")
        return true;
 ...
 */
export function safeFieldName(lang, name) {
 ...
 */
export function dottedPathToSnakeCase(path) {
    return path
        .split(".")
        .map((s) => toSnakeCase(s))
        .join("_");
 ...
 */
export function dottedPathToPascalCase(path) {
    return path
        .split(".")
...
export const RESERVED_KEYWORDS = {
    ...
    scala: new Set([
        ...
        "while", "with", "yield", "enum", "export", "given", "then",
        "using", "extension", "inline", "opaque", "open", "transparent",
        "derives", "end", "with", "_",
    ]),
    fsharp: new Set([
        ...
        "inline", "interface", "internal", "lazy", "let", "match", "member",
        "module", "mutable", "namespace", "new", "not", "null", "of", "open",
        "or", "override", "private", "public", "rec", "return", "select",
        "static", "struct", "then", "to", "true", "try", "type", "upcast",
        "use", "val", "void", "when", "while", "with", "yield",
    ]),
    elixir: new Set([
        "after", "alias", "and", "case", "catch", "cond", "def", "defmacro",
        "defmodule", "defp", "defstruct", "do", "else", "end", "false",
        "fn", "for", "if", "import", "in", "nil", "not", "or", "quote",
        "raise", "receive", "require", "rescue", "super", "then", "throw",
        "true", "try", "unless", "unquote", "use", "when", "__MODULE__",
        "__DIR__", "__ENV__", "__STACKTRACE__", "__CALLER__",
    ]),
};
...
export function formatReservedError(fieldName, _modelName, reservedIn) {
    const langNames = reservedIn.map((l) => LANG_DISPLAY_NAMES[l]);
    if (langNames.length === 1) {
 ...
 */
export function checkAndReportReservedKeywords(program, services, _ignoreReservedKeywords) {
    const warnings = [];
    for (const svc of services) {
